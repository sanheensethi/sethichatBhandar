...
?>
  ...
  <tbody>
      ...
      */
        try{
         $stmt = $db->prepare("SELECT * FROM orders WHERE date='".$date."' ORDER BY datetime DESC");
         $stmt->execute();
...
$data2="<p>Delivered</p>";
            ...
            echo '<td>
               <button class="btn btn-success" id="update'.$i.'">Update</button>
               <script>
                   $("#update'.$i.'").on("click",function(e){
                     e.preventDefault();
                     $("#orderid").val('.$results[$i]->orderID.');
                    });
               </script>
            </td>';
            echo "<td>".$results[$i]->mobile."</td>";
